"""System prompt construction for MewCode (ch05).

Static rules (identity, safety, tool usage, tone, output style) and environment
info are folded into the System Prompt so the model behaves predictably.
Dynamic instructions (Plan Mode reminders, hook injections, skill pulls) ride
the user channel as `<system-reminder>` blocks or `inject_environment`, so the
cacheable System Prompt prefix stays stable.
"""
...
# ---------------------------------------------------------------------------
# Plan Mode reminders (user channel, NOT in the system prompt)
# ---------------------------------------------------------------------------
...
def build_system_prompt(
    ...
    if coordinator_mode:
        try:
            ...
            pass  # teams module not present yet; fall through to normal prompt
